...
class React
{
    public static function enqueue($version = false)
    {
        ...
        if (!wp_script_is('@babel/polyfill')) {
            ...
            wp_enqueue_script('@babel/polyfill', 'https://cdn.jsdelivr.net/npm/@babel/polyfill@7.0.0/dist/polyfill' . $min . '.js', [], null);
        }

        //Enqueue react
        if (!wp_script_is('react')) {
            wp_enqueue_script('react', 'https://unpkg.com/react@' . $version . '/umd/react.' . $suffix . '.js', [], null);
        }

        if (!wp_script_is('react-dom')) {
            wp_enqueue_script('react-dom', 'https://unpkg.com/react-dom@' . $version . '/umd/react-dom.' . $suffix . '.js', ['react'], null);
        }
    }
}
