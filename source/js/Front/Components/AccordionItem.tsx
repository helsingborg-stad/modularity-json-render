	...
	<section className="accordion-section c-accordion__section">
		{view === 'accordiontable' && (
			...
			>
				<span className="c-accordion__button-wrapper">
					{heading.map((title, i) => (
						<span key={i} className={'match-heading'} tabIndex="-1">
							{heading[i]}
						</span>
					))}
		...
		{view === 'accordion' && (
			<button
				...
				htmlFor={'c-accordion__aria-jasonRender-' + index}
			>
				<span className="c-accordion__button-wrapper" tabIndex="-1">
					{heading[0]}
					<i className="c-icon c-accordion__icon c-icon--size-md material-icons">keyboard_arrow_down</i>
				</span>
			</button>
		...
		<div className="c-accordion__content" id={'c-accordion__aria-jasonRender-' + index} aria-hidden="true">
			{content
